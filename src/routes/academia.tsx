    ...
    <>
      <section className="relative isolate overflow-hidden">
        ...
        <div className="container-fortux relative py-20 md:py-28 text-primary-foreground">
          <span className="inline-block rounded-full bg-secondary px-3 py-1 text-xs font-semibold uppercase tracking-wider text-secondary-foreground">{t("aca.badge")}</span>
          <h1 className="mt-4 font-display text-4xl md:text-6xl font-bold text-balance max-w-2xl">{t("aca.hero")}</h1>
        </div>
      </section>
        ...
        <div className="container-fortux">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <Button asChild className="bg-primary hover:bg-primary-glow">
              <a href={waLink("Hola, querría reservar una clase.")} target="_blank" rel="noopener">{t("cta.bookClass")}</a>
            </Button>
            <Button asChild variant="outline">
              <a href={waLink("Hola, querría más información sobre la academia.")} target="_blank" rel="noopener">{t("cta.moreInfo")}</a>
            </Button>
          </div>
        </div>
      </section>
